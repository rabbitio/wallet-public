 ...
 * P2SH-P2WPKH to be precise should has other then 546 satoshi threshold but according to implementation (there is
...
export const MAX_BITCOIN_AMOUNT = 21000000;
 ...
 * Ignores:
 *   - utxos with less than MIN_CONFIRMATIONS number of confirmations (if address of utxo is external)
...
export function getAllSpendableUtxosByWalletData(accountsData, allUtxos, indexes, network) {
    try {
        const spendableExternalUtxos = allUtxos.external.filter(utxo => utxo.confirmations >= MIN_CONFIRMATIONS);
...
export function calculateDustBalanceByWalletData(allUTXOs, feeRate, network) {
    try {
        const relevantUTXOs = [
            ...allUTXOs.internal,
            ...allUTXOs.external.filter(utxo => utxo.confirmations > MIN_CONFIRMATIONS),
        ];
...
function calculateBalanceByUtxos(utxos, confirmedOnly) {
    ...
    return onlyNotDustUtxos.reduce((wholeSum, utxo) => {
        if (!confirmedOnly || (confirmedOnly && utxo.confirmations >= MIN_CONFIRMATIONS)) {
 ...
 * Returns only utxos corresponding to used addresses of given accountsData in given network.
 * Ignores:
 *   - external utxos with less than MIN_CONFIRMATIONS number of confirmations
...
/**
 * Checks whether given amount will became dust in terms of sending to segwit/non-segwit address type.
 *
...
export function isAmountDustForAddress(amount, address) {
    ...
    if (isSegWitAddress(address)) {
        return { result: amount < SEGWIT_DUST_THRESHOLD, threshold: SEGWIT_DUST_THRESHOLD };
    } else {
        return { result: amount < NON_SEGWIT_DUST_THRESHOLD, threshold: NON_SEGWIT_DUST_THRESHOLD };
    }
}
