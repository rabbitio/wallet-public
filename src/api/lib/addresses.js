...
export function isAddressValid(address, network) {
    try {
        ...
        bitcoinJs.address.toOutputScript(address, network.bitcoinjsNetwork);
        return true;
    } catch (e) {
        return false;
    }
...
export function getAddressByIndex(scheme, changeNode, addressIndex, network) {
    ...
    const ecPair = bitcoinJs.ECPair.fromPublicKey(Buffer.from(publicKey), network.bitcoinjsNetwork);
    ...
    try {
        ...
        changeNodes.forEach(changeNodeData => {
            for (let index = 0; index <= changeNodeData.index; ++index) {
                ...
                const ecPair = bitcoinJs.ECPair.fromPrivateKey(privateKey, { network: network.bitcoinjsNetwork });
...
export function getMappingOfAddressesToRandomEcPair(mappingAddresses, network) {
    try {
        const randomEcPair = bitcoinJs.ECPair.makeRandom({ network: network.bitcoinjsNetwork });
