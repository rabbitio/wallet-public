...
class CoincapCoinToUsdRateProvider extends ExternalApiProvider {
    ...
    getDataByResponse(response, params = [], subRequestIndex = 0, iterationsData = []) {
        try {
            ...
            return {
                coin: params[0],
                usdRate: rateData?.priceUsd ? +rateData.priceUsd : 1,
                timestamp: rateData?.time ?? Date.now(),
            };
        } catch (e) {
...
class CoingeckoCoinToUsdRateProvider extends ExternalApiProvider {
    ...
    getDataByResponse(response, params = [], subRequestIndex = 0, iterationsData = []) {
        try {
            ...
            return {
                coin: coin,
                usdRate: coinData?.market_data?.current_price?.usd ? +coinData?.market_data?.current_price?.usd : 1,
                timestamp: params[1] ?? Date.now(),
            };
        } catch (e) {
...
class MessariCoinToUsdRateProvider extends ExternalApiProvider {
    ...
    getDataByResponse(response, params = [], subRequestIndex = 0, iterationsData = []) {
        try {
            ...
            return {
                coin: params[0],
                usdRate: ((coinData?.values ?? [])[0] ?? [])[valueIndex] ? +coinData.values[0][valueIndex] : 1,
                timestamp: params[1] ?? Date.now(),
            };
        } catch (e) {
...
class CoinToUSDRateAtSpecificDateProvider {
    ...
    async getCoinToUSDRateAtSpecificDate(coin, timestamp) {
        try {
            ...
            // if (result?.usdRate == null) {
            //     const rate = await publicSwapServiceInstance.getAssetToUsdtRate(coin);
            //     result.usdRate = rate;
            // }
