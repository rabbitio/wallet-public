...
import { bip44Scheme, bip49Scheme, bip84Scheme, SupportedSchemes } from "./addresses-schemes.js";
...
export class BitcoinAddresses {
     ...
     */
    static isAddressValid(address, network = null) {
        try {
            if (!(network instanceof Network)) {
                network = BitcoinAddresses.getNetworkByAddress(address);
            }
     ...
     */
    static isSegWitAddress(address) {
     ...
     */
    static isP2wpkhAddress(address) {
        return (
            typeof address === "string" && address.length === 42 && (address.match(/^bc1q/g) || address.match(/^tb1q/g))
        );
    }
     ...
     * @returns {Network}
     */
    static getNetworkByAddress(address) {
     ...
     */
    static getAddressByIndex(scheme, changeNode, addressIndex, network) {
     ...
     */
    static createNewExternalAddressByScheme(accountsData, network, addressNodesIndexes, scheme, incrementWith = 1) {
        try {
            ...
            const newAddress = BitcoinAddresses.getAddressByIndex(
                scheme,
                externalChangeNode,
     ...
     */
    static isBip49Addresses(accountsData, addresses, addressIndexes, network) {
        try {
            ...
            [EXTERNAL_CHANGE_INDEX, INTERNAL_CHANGE_INDEX].forEach(changeIndex => {
                ...
                for (let addressIndex = 0; addressIndex <= currentAddressIndex; ++addressIndex) {
                    const bip49Address = BitcoinAddresses.getAddressByIndex(
    ...
    try {
        ...
        schemes.forEach(scheme => {
            ...
            for (let index = 0; index <= currentAddressIndex; ++index) {
                const address = BitcoinAddresses.getAddressByIndex(scheme, changeNode, index, network);
...
export class EcPairsUtils {
     ...
     */
    static getEcPairsToAddressesMapping(mappingAddresses, seedHex, network, indexes, schemes = SupportedSchemes) {
        try {
            const coinIndex = network.coinIndex;
            const accountIndex = network.defaultAccountIndex;
            const changeNodes = [];
     ...
     * @returns {EcPairsMappingEntry[]}
     */
    static getMappingOfAddressesToRandomEcPair(mappingAddresses, network) {
        try {
            const randomEcPair = bitcoinJs.ECPair.makeRandom({
                network: BitcoinJsAdapter.toBitcoinJsNetwork(network.key),
            });
