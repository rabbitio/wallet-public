...
import { BitcoinAddresses } from "./addresses.js";
import { getNotDustUTXOsInTermsOfSpecificFeeRateConsideringSendingP2WPKH } from "./transactions/transactions-utils.js";
...
export class Utxos {
     ...
     * @returns {Utxo[]}
     */
    static getAllSpendableUtxosByWalletData(accountsData, allUtxos, indexes, network) {
        try {
    ...
    static calculateBalanceByWalletData(accountsData, allUtxos, indexes, network) {
        try {
     ...
     */
    // TODO: [tests, low] Write unit tests for payment logic
    static calculateDustBalanceByWalletData(allUTXOs, feeRate, network) {
        try {
            const relevantUTXOs = [
                ...allUTXOs.internal,
            ...
            const dustUTXOs = relevantUTXOs.filter(
                utxo =>
                    !notDust.find(nonDustUTXO => nonDustUTXO.txid === utxo.txid && nonDustUTXO.number === utxo.number)
            );
     ...
     */
    // TODO: [tests, low] Write unit tests for payment logic
    static getSortedListOfCandidateUtxosForRbf(accountsData, indexes, candidateUtxos, network) {
        try {
     ...
     *
     * @param amount {number|string} amount to be checked (satoshi)
     * @param address {string} target address to send given amount to
     * @returns {{result: boolean, threshold: number }}
     */
    // TODO: [tests, low] Write unit tests for payment logic
    static isAmountDustForAddress(amount, address) {
        try {
            ...
            const dustThreshold = Utxos.getDustThreshold(address, false);
            return {
                result: BigNumber(amount).lt(dustThreshold),
    ...
    static getDustThreshold(targetAddress, validateAddress = true) {
        if (validateAddress && !BitcoinAddresses.isAddressValid(targetAddress)) {
            throw new Error(`Address is invalid: ${targetAddress}.`);
        }

        if (BitcoinAddresses.isSegWitAddress(targetAddress)) {
     ...
     */
    static getTXIDSendingGivenOutput(output, outputTxId, transactions) {
...
function getNotDustUtxos(utxos) {
    return utxos.filter(utxo => {
        ...
        if (utxo.type === P2WPKH_SCRIPT_TYPE || BitcoinAddresses.isSegWitAddress(utxo.address)) {
...
function getSignableUtxos(accountsData, utxos, indexes, network) {
    ...
    const isBip49AddressMapping = BitcoinAddresses.isBip49Addresses(accountsData, addresses, indexes, network);
...
export function getOutputTypeByAddress(address) {
    let type = null;
    if (BitcoinAddresses.isP2wpkhAddress(address)) {
        type = P2WPKH_SCRIPT_TYPE;
    } else if (BitcoinAddresses.isP2shAddress(address)) {
        type = P2SH_SCRIPT_TYPE;
    } else if (BitcoinAddresses.isP2pkhAddress(address)) {
