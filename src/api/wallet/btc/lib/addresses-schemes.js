 ...
 *
 * TODO: [refactoring, moderate] refactor to separate base and btc-dependent logic
 */
...
class Scheme {
    ...
    /**
     * Generates public, private key and chain code for desired account of given wallet.
     *
     * @param mnemonic {string} mnemonic phrase to generate account keys for
     * @param password {string} password for mnemonic phrase
     * @param coinIndex {number} index of desired coin in HD wallet
     * @param accountIndex{number} desired account index in HD wallet for specified coin
     *
     * @return {{publicKeyHex: string, privateKeyHex: string, chainCodeHex: string}}
     */
    generateAccountKeys(mnemonic, password, coinIndex, accountIndex) {
        ...
        return {
            ...
            privateKeyHex: Buffer.from(accountNode.privateKey).toString("hex"),
    ...
    deriveNeuteredChangeNodeForAccount(accountsData, network, accountIndex, changeIndex) {
        ...
        const accountNodeNeutered = bip32.fromPublicKey(
            publicKey,
            chainCode,
